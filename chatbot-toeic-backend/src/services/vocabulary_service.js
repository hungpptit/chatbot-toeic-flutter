  ...
  try {
    ...
    if (!vocab) {
      ...
      for (const p of phonetics) {
        ...
        uniquePhonetics.set(key, { phoneticText, audioUrl });
      }

      for (const { phoneticText, audioUrl } of uniquePhonetics.values()) {
        const accent = audioUrl?.includes('uk') ? 'UK'
                     : audioUrl?.includes('us') ? 'US'
                     : audioUrl?.includes('au') ? 'AU'
                     : 'Other';
        ...
        try {
          ...
          console.error('❌ Failed to save pronunciation:', { word, phoneticText, audioUrl, error: err.message });
        }
      }
      ...
      for (const syn of allSynonyms) {
        try {
          await Synonym.create({ vocabId: vocab.id, synonym: syn });
        } catch (err) {
      ...
      for (const ant of allAntonyms) {
        try {
          await Antonym.create({ vocabId: vocab.id, antonym: ant });
        } catch (err) {

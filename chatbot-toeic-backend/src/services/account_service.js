  ...
  try {
    ...
    if (data.password && data.password.trim() !== '') {
     ...
     if (data.email && data.email !== user.email) {
      ...
      await sendOTP(
        data.email,
        'Xác minh thay đổi email',
        `Mã OTP xác minh email mới của bạn là: ${otp} (hết hạn sau 5 phút).`
      );
...
const verifyEmailUpdateOtp = async (userId, email, otp) => {
  ...
  if (!entry) {
    return { code: 400, message: 'Không tìm thấy yêu cầu xác minh email.' };
  }

  if (Date.now() > entry.expiresAt) {
    otpStore.delete(key);
    return { code: 400, message: 'Mã OTP đã hết hạn.' };
  }

  if (entry.otp !== otp) {
    return { code: 400, message: 'Mã OTP không chính xác.' };
  }

  const user = await db.User.findByPk(userId);
  if (!user) return { code: 404, message: 'Người dùng không tồn tại.' };

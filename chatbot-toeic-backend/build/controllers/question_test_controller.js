import { RandomQuestionsByTestId, updateQuestion, SubmitTestResult, CheckUserHasDoneTestDetailed, GetUserTestDetailById, GetUserTestHistoryByTestId, StartUserTest, createQuestion } from '../services/question_test_service.js';
...
const startTest = async (req, res) => {
  try {
    ...
    const result = await StartUserTest({
      userId,
      testId
    });
...
const checkUserTestDetailed = async (req, res) => {
  try {
    ...
    const result = await CheckUserHasDoneTestDetailed({
      userId,
      testId
    });
    res.status(200).json(result);
...
const checkHistoryUserTestDetailed = async (req, res) => {
  try {
    ...
    const result = await GetUserTestHistoryByTestId({
      userId,
      testId
    });
...
export { getQuestionsByTest, updateQuestionController, submitTest, checkUserTestDetailed, getUserTestDetailId, checkHistoryUserTestDetailed, startTest, createQuestionController };
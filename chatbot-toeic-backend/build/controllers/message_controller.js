import { createMessage, getMessagesByConversation, getMessagesForGemini } from '../services/message_service.js';
...
const handleCreateMessage = async (req, res) => {
  ...
  if (!conversationId || !role || !content) {
    return res.status(400).json({
      message: "Thiếu dữ liệu: conversationId, role, content"
    });
  }
...
const handleGetMessagesByConversation = async (req, res) => {
  ...
  if (!conversationId) {
    return res.status(400).json({
      message: "Thiếu conversationId trong URL"
    });
  }
...
export { handleCreateMessage, handleGetMessagesByConversation, handleGetMessagesForGemini };
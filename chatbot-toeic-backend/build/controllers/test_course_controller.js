import { getAllTestsWithCourses, getAllCourseNames, getAllCoursesWithTests, updateCourseName, deleteCourseById, insertCourse } from "../services/test_course_service.js";
const getAllTestsWithCoursesController = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Error fetching tests with courses"
    });
  }
};
...
const getCoursesNameWithTests = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Error fetching courses with tests"
    });
  }
};
const updateCourseNameController = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Error updating course name"
    });
  }
};
...
export { getAllTestsWithCoursesController, getAllCourseNamesController, getCoursesNameWithTests, updateCourseNameController, deleteCourseByIdController, insertCourseController };
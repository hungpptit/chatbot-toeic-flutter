...
import { getAllTestsWithCourses, getAllQuestionTypes, getAllParts, createPart, createQuestionType, deletePart, deleteQuestionType, createNewTest, updatePartName, updateQuestionType, deleteTestById } from '../services/AdminTest_service.js';
...
const updateQuestionTypeController = async (req, res) => {
  try {
    ...
    if (!name || typeof name !== "string") {
      return res.status(400).json({
        message: "Tên loại câu hỏi không hợp lệ."
      });
    }
    ...
    res.status(500).json({
      message: "Cập nhật question type thất bại."
    });
  }
};
export { getTestList, getQuestionTypes, getParts, createPartController, deletePartController, createQuestionTypeController, deleteQuestionTypeController, createNewTestController, updatePartNameController, updateQuestionTypeController, deleteTestByIdController };
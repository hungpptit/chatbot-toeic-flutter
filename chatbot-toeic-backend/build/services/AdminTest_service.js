...
const getAllTestsWithCourses = async () => {
  try {
    ...
    const formattedTests = testList.map(test => ({
      ...
      // đếm từ bảng trung gian TestQuestions
...
const deleteTestById = async testIdRaw => {
  try {
    ...
    await db.Test_Courses.destroy({
      where: {
        testId
      }
    });
    await db.UserTest.destroy({
      where: {
        testId
      }
    });
    ...
    return {
      success: true,
      message: `Test ID ${testId} deleted`
    };
  } catch (error) {
...
export { getAllTestsWithCourses, getAllQuestionTypes, getAllParts, createPart, createQuestionType, deletePart, deleteQuestionType, createNewTest, updatePartName, updateQuestionType, deleteTestById };
...
const parseUserInput = async (rawText, conversationId) => {
  const {
    data: history
  } = await getMessagesForGemini(conversationId); // Lấy lịch sử 15 cặp
...
Chuỗi mới:
"""${rawText}"""`;
  const reply = await callGemini(history.concat({
    role: "user",
    parts: [{
      text: prompt
    }]
...
const detectQuestionType = async (questionText, options, conversationId) => {
  const {
    data: history
  } = await getMessagesForGemini(conversationId);
...
const askFreeQuestion = async (questionText, conversationId) => {
  ...
  return await callGemini(history.concat({
    role: "user",
    parts: [{
      text: prompt
    }]
...
const askWithLocalAI = async (questionText, options, type = 'Part 5', conversationId) => {
  ...
  const intro = type === 'Vocabulary' ? 'Bạn là trợ lý luyện từ vựng tiếng Anh. Hãy chọn đáp án đúng và giải thích bằng tiếng Việt.' : 'Bạn là trợ lý tiếng Anh. Hãy chọn đáp án đúng và giải thích bằng tiếng Việt.';
  const prompt = `${intro}
...
const getItemWithAI = async ({
  ...
  if (type === 'Vocabulary-Lookup') {
    ...
    if (!vocab) {
      ...
      for (const s of aiData.synonyms) await Synonym.create({
        vocabId: vocab.id,
        synonym: s
      });
      for (const a of aiData.antonyms) await Antonym.create({
        vocabId: vocab.id,
        antonym: a
      });
    }

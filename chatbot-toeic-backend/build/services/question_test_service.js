...
export const RandomQuestionsByTestId = async (testId, limit = 40) => {
  try {
    ...
    const shuffled = testQuestionRows.sort(() => Math.random() - 0.5).slice(0, limit);
...
export const SubmitTestResult = async ({
  ...
  return await db.sequelize.transaction(async transaction => {
    ...
    if (resultsToSave.length > 0) {
      await db.UserResult.bulkCreate(resultsToSave, {
        transaction
      });
    }
...
export const StartUserTest = async ({
  userId,
  testId
}) => {
...
export const CheckUserHasDoneTestDetailed = async ({
  userId,
  testId
}) => {
  try {
    ...
    return {
      ...
      message: userTestRecord.status === 'completed' ? 'User has completed this test.' : 'User has started but not completed this test.'
    };
  } catch (error) {
...
export const GetUserTestDetailById = async userTestId => {
  try {
    ...
    for (const result of userResults) {
      ...
      if (isSkipped) skippedCount++;else if (result.isCorrect) correctCount++;else incorrectCount++;
      details.push({
...
export const GetUserTestHistoryByTestId = async ({
  userId,
  testId
}) => {
  try {
    ...
    for (const test of userTests) {
      ...
      const completed = test.completedAt ? dayjs(test.completedAt).tz(VN_TIMEZONE) : started; // Nếu chưa có completedAt thì để bằng started để tránh sai lệch

      const durationInSec = completed.diff(started, 'second');
      const formattedDuration = new Date(durationInSec * 1000).toISOString().substr(11, 8); // HH:mm:ss
